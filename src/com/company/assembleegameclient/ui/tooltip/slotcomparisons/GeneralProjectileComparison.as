    ...
    public class GeneralProjectileComparison extends SlotComparison
    {
        ...
        private function addProjectileText():void
        {
            ...
            comparisonStringBuilder.pushParams(
                    TextKey.RANGE, {"range": wrapInColoredFont(_local3, getTextColor((_local1 - _local2)))}
            );
            if (this.projXML.hasOwnProperty("MultiHit"))
            {
                comparisonStringBuilder.pushParams(
                        TextKey.MULTIHIT, {}, TooltipHelper.getOpenTag(NO_DIFF_COLOR), TooltipHelper.getCloseTag()
                );
            }
            if (this.projXML.hasOwnProperty("PassesCover"))
            {
                comparisonStringBuilder.pushParams(
                        TextKey.PASSES_COVER, {}, TooltipHelper.getOpenTag(NO_DIFF_COLOR), TooltipHelper.getCloseTag()
                );
            }
            if (this.projXML.hasOwnProperty("ArmorPiercing"))
            {
                comparisonStringBuilder.pushParams(
                        TextKey.ARMOR_PIERCING, {}, TooltipHelper.getOpenTag(NO_DIFF_COLOR), TooltipHelper.getCloseTag()
                );
            }
        }
        ...
        private function addNumProjectileText():void
        {
            ...
            comparisonStringBuilder.pushParams(
                    TextKey.SHOTS, {"numShots": wrapInColoredFont(_local1.toString(), _local3)}
            );
        }
        ...
        private function addDamageText():void
        {
            ...
            comparisonStringBuilder.pushParams(
                    TextKey.DAMAGE, {"damage": wrapInColoredFont(_local7, getTextColor((_local3 - _local6)))}
            );
        }
