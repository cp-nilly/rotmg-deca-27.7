package com.company.assembleegameclient.background
{
    ...
    public class StarBackground extends Background
    {
        ...
        private function tryAddStar():void
        {
            ...
            var _local2:Star = new Star(
                    ((Math.random() * 1000) - 500),
                    ((Math.random() * 1000) - 500),
                    (4 * (0.5 + (0.5 * Math.random()))),
                    _local1.images_[int((_local1.images_.length * Math.random()))]
            );
            for each (_local3 in this.stars_)
            {
...
class Star
{
    protected static const sqCommands:Vector.<int> = new <int>[
