package kabam.rotmg.questrewards.view
{
    ...
    public class QuestRewardsMediator extends Mediator 
    {
        ...
        private function onInit(_arg1:TimerEvent):void
        {
            if (Math.random() < 0.5)
            {
        ...
        {
            this.targetItem = int(_arg1.goal);
            var _local2:int = _arg1.tier;
            if (_local2 > 0)
            {
                this.view.init(int(_arg1.tier), this.targetItem, _arg1.description, _arg1.image);
                this.setupEvents();
            }
            else
            {
        ...
        private function onNewItem(_arg1:int):void
        {
            if (((!((this.itemslot == null))) && (!(this.state_tinkering))))
            {
                if (_arg1 == this.targetItem)
                {
        ...
        private function onExchangeClick(_arg1:MouseEvent):void
        {
            if (this.gsc != null)
            {
                this.exchangeButton.removeEventListener(MouseEvent.CLICK, this.onExchangeClick);
